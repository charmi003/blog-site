...
export const fetchUsers=()=>{
    return (dispatch)=>{
        axios.get('http://localhost:8000/admin/all-users',{
            'headers':{
...
export const deleteUser=(userId,alert)=>{
    return (dispatch)=>{
        axios.get(`http://localhost:8000/admin/delete-user/${userId}`,{
            'headers':{
                'x-access-token':window.localStorage.token
            }
        }).then((response)=>{
...
export const addContentWriter=(data,alert,setUsername,setPassword)=>{
    return (dispatch)=>{
        ...
        axios.post('http://localhost:8000/admin/add-content-writer',JSON.stringify(dataObj),config).then((response)=>{
