...
export const createBlog=(data,alert,navigate)=>{
    return (dispatch,getState)=>{
        ...
        axios.post('http://localhost:8000/blogs/create',JSON.stringify(dataObj),config).then((response)=>{
...
export const fetchBlogs=()=>{
    return (dispatch)=>{
        axios.get('http://localhost:8000/blogs/all-blogs').then((response)=>{
...
export const fetchMyBlogs=()=>{
    return (dispatch,getState)=>{
            axios.get(`http://localhost:8000/blogs/my-blogs/${getState().auth.user._id}`,{
                'headers':{
                    'x-access-token':window.localStorage.token
                }
            }).then((response)=>{
...
export const deleteBlog=(blogId,alert)=>{
    return (dispatch)=>{
        axios.get(`http://localhost:8000/admin/delete-blog/${blogId}`,{
            'headers':{
                'x-access-token':window.localStorage.token
            }
        }).then((response)=>{
...
export const approveBlog=(blogId,alert)=>{
    return (dispatch)=>{
        axios.get(`http://localhost:8000/admin/approve-blog/${blogId}`,{
            'headers':{
                'x-access-token':window.localStorage.token
            }
        }).then((response)=>{
